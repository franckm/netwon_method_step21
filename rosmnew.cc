...
namespace Step21
{
  ...
  private:
    ...
    //void assemble_rhs_S ();
    void assemble_system_HS();
    void solve();

    void ROSENBROCK_METHOD (const unsigned int n_time_steps,
        		            const double intial_time,
                            const double final_time);

    void NEWTON_METHOD(const unsigned int n_time_steps,
        		       const double intial_time,
                       const double final_time);

    //BlockVector<double> FV (const double time_step, BlockVector<double>&);
    BlockVector<double> FV (BlockVector<double>&);
    BlockVector<double> LV (BlockVector<double>&);

    BlockVector<double> source (BlockVector<double>&);
    ...
    BlockSparseMatrix<double> system_matrix, system_matrix_L, L_plus_gamma_jacobian, L_minus_delta_t_jacobian;

    SparseDirectUMFPACK  inverse_L_minus_gamma_jacobian;
    SparseDirectUMFPACK  inverse_L_minus_delta_t_jacobian;
    ...
    BlockVector<double> solution, vn;
    //BlockVector<double> old_solution;
    BlockVector<double> system_rhs;
    BlockVector<double> r_h_s, L_s;
    BlockVector<double> F_V, L_V, delta_V;
  };
  ...
  TwoPhaseFlowProblem<dim>::TwoPhaseFlowProblem (const unsigned int degree)
    ...
    /*c{0.8786796564403575,0.2928932188134525},
    alpha{0,0,0.5857864376269050,0},
    stage (2)*/
  {}
  ...
  void TwoPhaseFlowProblem<dim>::make_grid_and_dofs ()
  {
    ...
    system_matrix_L.reinit (sparsity_pattern);
    L_plus_gamma_jacobian.reinit (sparsity_pattern);
    L_minus_delta_t_jacobian.reinit(sparsity_pattern);
    ...
    /*solutionr.reinit (3);
    solutionr.block(0).reinit (n_u);
    solutionr.block(1).reinit (n_p);
    solutionr.block(2).reinit (n_s);
    solutionr.collect_sizes ();*/

    /*v0.reinit (3);
    v0.block(0).reinit (n_u);
    v0.block(1).reinit (n_p);
    v0.block(2).reinit (n_s);
    v0.collect_sizes ();*/

    vn.reinit (3);
    vn.block(0).reinit (n_u);
    vn.block(1).reinit (n_p);
    vn.block(2).reinit (n_s);
    vn.collect_sizes ();

    /*vnew.reinit (3);
    vnew.block(0).reinit (n_u);
    vnew.block(1).reinit (n_p);
    vnew.block(2).reinit (n_s);
    vnew.collect_sizes ();*/
  ...
  void TwoPhaseFlowProblem<dim>::assemble_system ()
  {
    ...
    std::vector<std::vector<Tensor<1,dim> > >  solution_grads(n_q_points,
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        for (unsigned int q=0; q<n_q_points; ++q)
          for (unsigned int i=0; i<dofs_per_cell; ++i)
            {
              const double sat_q = solution_values[q](dim+1);
              ...
              for (unsigned int j=0; j<dofs_per_cell; ++j)
                {
                  ...
                  local_matrix(i,j) += (phi_i_u * k_inverse_values[q] *
                                        mobility_inverse(sat_q,viscosity) * phi_j_u
                                        - div_phi_i_u * phi_j_p
                                        - phi_i_p * div_phi_j_u)
                                        //+ phi_i_s * phi_j_s)
                                       * fe_values.JxW(q);
...
*/
  ...
  void TwoPhaseFlowProblem<dim>::assemble_system ()
  {
    ...
    FullMatrix<double>   local_matrix (dofs_per_cell, dofs_per_cell);
    FullMatrix<double>   local_matrix_L (dofs_per_cell, dofs_per_cell);
    Vector<double>       local_rhs (dofs_per_cell);
    ...
    for (; cell!=endc; ++cell)
      {
              ...
              for (unsigned int j=0; j<dofs_per_cell; ++j)
                {
                  ...
                  local_matrix(i,j) += (phi_i_u * k_inverse_values[q] *
                                        ...
                                        //+ phi_i_s * phi_j_s)
                                       * fe_values.JxW(q);
                  local_matrix_L(i,j) += (phi_i_s * phi_j_s)
                                         * fe_values.JxW(q);
                }
        ...
        for (unsigned int i=0; i<dofs_per_cell; ++i)
        	for (unsigned int j=0; j<dofs_per_cell; ++j)
        		system_matrix_L.add (local_dof_indices[i],
        				             local_dof_indices[j],
                                     local_matrix_L(i,j));
   ...
   //L_plus_gamma_jacobian.copy_from(system_matrix_L);
  }
...
template <int dim>
  void TwoPhaseFlowProblem<dim>::assemble_system_HS ()
   {
     QGauss<dim>   quadrature_formula(degree+2);
     QGauss<dim-1> face_quadrature_formula(degree+2);
     ...
     FullMatrix<double>   local_matrix (dofs_per_cell, dofs_per_cell);
     FullMatrix<double>   local_matrix_div (dofs_per_cell, dofs_per_cell);
     ...
     // there is no more old_solution, old_solutions_values, present_solution... I only have in input vin_U_P_S blockvector


     std::vector<Vector<double> > solution_values(n_q_points, Vector<double>(dim+2));
     std::vector<Vector<double> > solution_values_face(n_face_q_points, Vector<double>(dim+2));
     std::vector<Vector<double> > solution_values_face_neighbor(n_face_q_points, Vector<double>(dim+2));
     ...
     std::vector<std::vector<Tensor<1,dim> > >  solution_grads(n_q_points, std::vector<Tensor<1,dim> > (dim+2));
     ...
     for (; cell!=endc; ++cell)
       {
         ...
         //fe_values[saturation].get_function_values (solution, local_saturation_values);
         fe_values.get_function_values (solution, solution_values);
         ...
         for (unsigned int q=0; q<n_q_points; ++q)
           ...
           for (unsigned int i=0; i<dofs_per_cell; ++i)
             {
               const double sat_q_points = solution_values[q](dim+1);

               /*Tensor<1,dim> velocity_q_points;
               for (unsigned int d=0; d<dim; ++d)
                  velocity_q_points[d] = solution_values[q](d)*/
              ...
              //const Tensor<1,dim> grad_phi_i_s = fe_values[saturation].gradient (i, q);
               ...
               for (unsigned int j=0; j<dofs_per_cell; ++j)
                 {
                   ...
                   local_matrix(i,j) += -fractional_flow(sat_q_points,viscosity) *
                                         phi_i_u *
                                         grad_phi_j_s
                                         * fe_values.JxW(q);

                   /*local_matrix_div(i,j) += -fractional_flow_derivative (sat_q_points,viscosity) *
                                            phi_i_u *
                                            grad_phi_j_s
                                            * fe_values.JxW(q);*/     //+ phi_i_s * phi_j_s) // I do not need    + phi_i_s * phi_j_s
               ...
               // I don't need time_step so I remove it
               //local_HS(i) += (fractional_flow(sat_q_points,viscosity) *
                              //velocity_q_points *
                              //grad_phi_i_s)*fe_values.JxW(q);
         ...
         for (unsigned int face_no=0; face_no<GeometryInfo<dim>::faces_per_cell; ++face_no)
             {
                   fe_face_values.reinit (cell, face_no);
                    ...
                    //fe_face_values[saturation].get_function_values (solution, local_saturation_values_face);
                   ...
                   if (cell->at_boundary(face_no))
                   {
                 	//pressure_boundary_values.value_list (fe_face_values.get_quadrature_points(), boundary_values);
                      ...
                      // dealing with the pressure boundary value
                      /*for (unsigned int q=0; q<n_face_q_points; ++q)
                      {
 							 for (unsigned int i=0; i<dofs_per_cell; ++i)
 							   {
                    ...
                    else
                      {
                        ...
                        fe_face_values_neighbor.get_function_values (solution, solution_values_face_neighbor);

                        for (unsigned int q=0; q<n_face_q_points; ++q)
                     	  neighbor_saturation[q] = solution_values_face_neighbor[q](dim+1);
                          //neighbor_saturation[q] = saturation_values_face_neighbor[q];
                          //neighbor_saturation[q] = solution[q](dim+1);
                      }


                    for (unsigned int q=0; q<n_face_q_points; ++q)
                      {
                        //Tensor<1,dim> velocity_u_face;
                        //for (unsigned int d=0; d<dim; ++d)
                        //  velocity_u_face[d] = solution_values_face[q](d);

                        fe_values[velocities].get_function_values (solution, local_velocity_values);

                        const double normal_flux = local_velocity_values[q] *
                                                   fe_face_values.normal_vector(q);
                        ...
                        for (unsigned int i=0; i<dofs_per_cell; ++i)
                     	   for (unsigned int j=0; j<dofs_per_cell; ++j)
                     		   ...
                     		   local_matrix(i,j) +=   (fe_face_values.normal_vector(q) *
                     				                  fe_face_values[saturation].value (i,q) *
                     		                          fe_face_values[velocities].value(j,q))*
                                                      fractional_flow(solution_values_face[q](dim+1),viscosity)*
                                                      fe_face_values.JxW(q);

                        /*for (unsigned int i=0; i<dofs_per_cell; ++i)
                        	for (unsigned int j=0; j<dofs_per_cell; ++j)

                               local_matrix_div(i,j) += (fe_face_values.normal_vector(q) *
      				                                     fe_face_values[saturation].value (i,q) *
      		                                             fe_face_values[velocities].value(j,q))*
                                                         fractional_flow_derivative(solution_values_face[q](dim+1),viscosity)*
                                                         fe_face_values.JxW(q);*/
                     	   ...
                     	   for (unsigned int i=0; i<dofs_per_cell; ++i)
                     		   for (unsigned int j=0; j<dofs_per_cell; ++j)
                     			   ...
                     			   local_matrix(i,j) += (fe_face_values.normal_vector(q) *
                     					                  fe_face_values[saturation].value (i,q) *
                     	                       		      fe_face_values[velocities].value(j,q))*
                     	                                  fractional_flow(neighbor_saturation[q],viscosity)*
                     	                                  fe_face_values.JxW(q);
                     	  ...
                     	  /*for (unsigned int i=0; i<dofs_per_cell; ++i)
                     		  for (unsigned int j=0; j<dofs_per_cell; ++j)
                     	           ...
                     	           local_matrix_div(i,j) += (fe_face_values.normal_vector(q) *
                     	                       		     fe_face_values[saturation].value (i,q) *
                     	                       	         fe_face_values[velocities].value(j,q))*
                     	                       	         fractional_flow_derivative(neighbor_saturation[q],viscosity)*
                     	                       	         fe_face_values.JxW(q);*/
         ...
         /*for (unsigned int i=0; i<dofs_per_cell; ++i)
        	 for (unsigned int j=0; j<dofs_per_cell; ++j)
        		 system_matrix_derivative.add (local_dof_indices[i],
                                               local_dof_indices[j],
                                               local_matrix_div(i,j));*/
     ...
     //system_matrix.print_formatted(std::cout,2,true,2,"0",1);
     //system_matrix_derivative.print_formatted(std::cout,2,true,2,"0",1);
     //system_matrix.block(0,2).copy_from(system_matrix_derivative.block(0,0));

     //std::cout << system_matrix_derivative.block(0,0).m() << std::endl;
     //std::cout << system_matrix_derivative.block(0,0).n() << std::endl;




     //system_rhs.block(2).print(std::cout);
...
/*  template <int dim>
          ...
          for (unsigned int i=0; i<dofs_per_cell; ++i)
            {
              ...
              for (unsigned int d=0; d<dim; ++d)
                present_u[d] = solution_values[q](d);

              //const double        phi_i_s      = fe_values[saturation].value (i, q);
              const Tensor<1,dim> grad_phi_i_s = fe_values[saturation].gradient (i, q);

              local_rhs(i) += (fractional_flow(sat_q,viscosity) *
                               present_u *
                               grad_phi_i_s)
            ...
            for (unsigned int q=0; q<n_face_q_points; ++q)
              {
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                  local_rhs(i) -= normal_flux *
                                  fractional_flow((is_outflow_q_point == true
                                                   ?
                                                   solution_values_face[q](dim+1)
                                                   :
  ...
  void TwoPhaseFlowProblem<dim>::solve ()
  {
    ...
    // two equations:
    {
      ...
      //SolverControl solver_control (solution.block(1).size(), 1e-24*schur_rhs.l2_norm());
      SolverControl solver_control (1000, 1e-24*schur_rhs.l2_norm());
      SolverCG<>    cg (solver_control);
    ...
    // Now the velocity:
    {
      ...
      //solution.block(2).print(std::cout);
  ...
  // results, set the final time at the end of the do loop to 250. template<int dim>

  template<int dim>
  BlockVector<double> TwoPhaseFlowProblem<dim>::source(BlockVector<double>& vec)
   {
     BlockVector<double> tempo(vec);
     tempo=0;
     system_matrix.vmult(tempo,vec);
     system_rhs.add(-1,tempo);
     //system_rhs.print(std::cout);
     return system_rhs;
    }
  ...
  template<int dim>
  BlockVector<double> TwoPhaseFlowProblem<dim>::FV(BlockVector<double>& vec)
   {
	  BlockVector<double> temp5(vec);
	  temp5=0;
	  system_matrix.vmult(temp5,vec);
	  system_rhs.add(-1,temp5);

	  /*system_matrix_L.vmult(temp2,vec);
	  temp2.add(-time_step,system_rhs);
	  return temp2;*/
      return system_rhs;
   }

  template<int dim>
    BlockVector<double> TwoPhaseFlowProblem<dim>::LV(BlockVector<double>& vec)
     {
	  BlockVector<double> temp2(vec);
	  temp2 = 0;
  ...
  template<int dim>
  void TwoPhaseFlowProblem<dim>::NEWTON_METHOD (const unsigned int  n_time_steps,
      ...
      do{
    	  ...
    	  //double fail = 0;
          double qt;
    	  // we solving using Newton method an equation of the form
    	  //G(X) = LX- delta t * f(X) - LV^n =0,  n = 0,1,2....
    	  vn =  solution;
    	  BlockVector<double> temp3(vn);
    	  temp3 = 0;
    	  system_matrix_L.vmult(temp3,vn);

    	  BlockVector<double> vnew(vn);
    	  vnew = 0;

    	  F_V = FV(vn);
    	  L_V = LV(vn);
    	  L_V.add(-time_step,F_V);
    	  L_V.add(-1,temp3);
    	  L_V.operator*=(-1); // -G(X)

          unsigned int iter = 0;
    	  qt = 1; // L_V.linfty_norm();

        	while (qt > tol)
        	{
        		// computing the jacobian  of the linear ssystem to be solved for delta_V
        		L_minus_delta_t_jacobian.copy_from(system_matrix_L);
        		L_minus_delta_t_jacobian.add(time_step,system_matrix);

        		iter = iter +1;
        		solve2();
        		vnew.sadd(1,vn,1,delta_V);
  ...
  /*template<int dim>
  void TwoPhaseFlowProblem<dim>::ROSENBROCK_METHOD (const unsigned int  n_time_steps,
                                                    const double initial_time,
                                                    const double final_time)
    { const double time_step = (final_time-initial_time)/static_cast<double> (n_time_steps);

      double time = initial_time;
      double c[2] {0.8786796564403575,0.2928932188134525};
      double alpha[2][2] {0,0,0.5857864376269050,0};
      unsigned int stage= 2;
      double gamma = 1.707106781186547;
      ...
      system_matrix.block(2,0).operator*=(time_step);
      L_plus_gamma_jacobian.copy_from(system_matrix_L);
      L_plus_gamma_jacobian.add(gamma,system_matrix);
      ...
      do{
        ...
        BlockVector<double> local_sum(solution);
           	 local_sum =0;
       for (unsigned int i=0; i<= stage-1; ++i)
       {
           	unsigned int j=0;
         ...
         if (j==i)
       {
              ...
              r_h_s = source(solution);

              r_h_s.block(2).operator*=(time_step);

              solve1();

              local_sum.sadd(1,c[i],L_s);
              //solution.add(1,local_sum);
              //L_s.print(std::cout);
              //local_sum.block(2).print(std::cout);
         ...
         {
               BlockVector<double> temp1(solution);
               temp1=0;
               BlockVector<double> abc(solution);
               abc=0;
               system_matrix.vmult(temp1,L_s);

               abc.sadd(s,a,solution,-alpha[i][j],L_s);
               r_h_s = source(abc);
               r_h_s.block(2).operator*=(time_step);

               r_h_s.sadd(1,-alpha[i][j],temp1);
               solve1();
               local_sum.sadd(1,c[i],L_s);
               //L_s.print(std::cout);
               //solution.add(1,local_sum);
...
*/
  ...
  template <int dim>
  void TwoPhaseFlowProblem<dim>::solve1 ()
    { inverse_L_minus_gamma_jacobian.initialize(L_plus_gamma_jacobian);
      inverse_L_minus_gamma_jacobian.vmult(L_s,r_h_s);
      //SolverControl           solver_control (1000, 1e-12);
      //SolverCG<>              solver (solver_control);
      //solver.solve (L_minus_gamma_jacobian, L_s, r_h_s,PreconditionIdentity());
    }

  template <int dim>
  void TwoPhaseFlowProblem<dim>::solve2 ()
     { inverse_L_minus_delta_t_jacobian.initialize(L_minus_delta_t_jacobian);
       inverse_L_minus_delta_t_jacobian.vmult(delta_V,L_V);


       /*SolverControl           solver_control (1000, 1e-12);
       SolverCG<>              solver (solver_control);
       solver.solve (L_minus_delta_t_jacobian, delta_V, F_V, PreconditionIdentity());*/
     }
  ...
  void TwoPhaseFlowProblem<dim>::run ()
  {
    ...
    //const unsigned int stage =2;
    const unsigned int n_time_steps = 100000;
    const double initial_time = 0.;
    const double final_time =100;
    //ROSENBROCK_METHOD(n_time_steps, initial_time, final_time);
    NEWTON_METHOD(n_time_steps, initial_time, final_time);
